...
static void InsertVertex(const vec3& position, const vec3& normal, const vec2& uv, const vec3& binormal, const vec3& tangent, const vec4& jointWeights, const vec4& jointIndices)
{
	Vertex vertex = { position, normal, uv, binormal, tangent, jointWeights, jointIndices };
...
vec4 ReadJointWeight(FbxMesh* mesh, int controlPointIndex, int vertexCounter)
{
	return vec4(1.0f, 1.0f, 1.0f, 1.0f);
}

vec4 ReadJointIndex(FbxMesh* mesh, int controlPointIndex, int vertexCounter)
{
	return vec4(0, 0, 0, 0);
}

// Save up data into GPU buffers.
bool ProcessMesh(FbxNode* node)
{
	...
	for (uint i = 0; i < triCount; i++)
	{
		...
		for (uint j = 0; j < 3; j++)
		{
			...
			vec4 jointWeights = ReadJointWeight(mesh, controlPointIndex, vertexCounter);
			vec4 jointIndices = ReadJointIndex(mesh, controlPointIndex, vertexCounter);
			vec2 uv = ReadUV(mesh, controlPointIndex, mesh->GetTextureUVIndex(i, j), 0);

			InsertVertex(position, normal, uv, binormal, tangent, jointWeights, jointIndices);
			vertexCounter++;
		}
	}
