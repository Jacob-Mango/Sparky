...
float Attenuate(Light light)
{
	...
	return light.intensity * 1.0 / (dot(direction, light.direction) + 0.01);
}

void main()
{
	u_LightSetup[0].position = vec3(0, 10, 0);
	u_LightSetup[0].intensity = 1.0;
	u_LightSetup[0].color = vec3(0.8, 0.8, 0.8);
