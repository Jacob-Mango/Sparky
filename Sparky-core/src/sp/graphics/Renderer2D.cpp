...
#define RENDERER_MAX_SPRITES	5000
...
#define RENDERER_MAX_TEXTURES	32 - 1
		...
		void Renderer2D::DrawLine(float x0, float y0, float x1, float y1, uint color, float thickness)
		{
			...
			vec2 normal = vec2(y1 - y0, -(x1 - x0)).Normalise().Multiply(thickness);
		...
		{
			DrawRect(rectangle.GetMinimumBound(), (vec2) rectangle.size * 2.0f, color);
		}
		...
		{
			FillRect(rectangle.GetMinimumBound(), (vec2) rectangle.size * 2.0f, color);
		}
		...
		void Renderer2D::Present()
		{
			...
			m_VertexArray->Bind();
			m_IndexBuffer->Bind();
			m_VertexArray->Draw(RenderType::TRIANGLES, m_IndexCount);
