...
namespace sp {
	namespace debug {
		...
		void DebugLayer::OnInit(graphics::Renderer2D& renderer, graphics::Material& material)
		{
			...
			m_UPSLabel = spnew Label("", 31.5f, 16.8f, FontManager::Get(24), 0xffffffff, Label::Alignment::RIGHT);
			m_FrametimeLabel = spnew Label("", 31.5f, 16.1f, FontManager::Get(24), 0xffffffff, Label::Alignment::RIGHT);
			m_MemoryUsageLabel = spnew Label("", 31.5f, 15.4f, FontManager::Get(24), 0xffffffff, Label::Alignment::RIGHT);
		...
		void DebugLayer::OnTick()
		{
			...
			m_UPSLabel->SetText(StringFormat::ToString(m_Application.GetUPS()) + " ups");
