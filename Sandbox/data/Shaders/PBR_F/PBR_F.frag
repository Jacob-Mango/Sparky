...
vec3 IBL(vec3 eye)
{
	...
	vec3 result = pow(cs.xyz, vec3(GAMMA)) * RadianceIBLIntegration(NdotV, g_Material.roughness, vec3(g_Material.specular));
...
{
	//vec3 direction = light.position - g_Attributes.position;
	//float multiplier = 1.0 / (dot(direction, direction) + 0.01);
...
vec3 NormalMap()
{
	...
	// normalMap = g_Attributes.normal * normalMap.rgb;
...
void main()
{
	...
	vec3 eye = normalize(sys_CameraPosition - g_Attributes.position);

	vec3 specMet = GetSpecular();

	g_Material.albedo = GetAlbedo();
	g_Material.specular = specMet.r;
	g_Material.roughness = GetRoughness();
	...
	for (int i = 0; i < 1; i++)
	{
		Light light = TestLight(); // sys_LightSetup[i];
