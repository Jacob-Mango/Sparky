...
vec3 Ambient(vec3 N, vec3 V, vec3 F0, vec3 R) 
{
    ...
    vec3 diffuse = irradiance * g_Material.albedo.rgb;
    vec3 ambient = (kD * diffuse);

    const float MAX_REFLECTION_LOD = 4.0;
    vec3 prefilteredColor = vec3(0.03); // textureLod(u_EnvironmentMap, R, g_Material.roughness * MAX_REFLECTION_LOD).rgb;    
    vec2 brdf = texture(u_PreintegratedFG, vec2(max(dot(N, V), 0.0), g_Material.roughness)).rg;
